    ...
    pub fn from_midi(m: &[u8]) -> Result<(Self, usize), ParseError> {
        if m.len() >= 3 {
            ...
            let model = match m[1] {
                ...
                n => return Err(ParseError::Invalid(format!("Bad B-Control model number. ({n:x})")))
            };
            let (command, used) = match m[2] {
                ...
                0x20 => (BControlCommand::SendBclMessage { text: string_from_midi(&m[3..])? }, m.len()),
                0x21 => {
                    if m.len() > 6 {
                ...
                0x34 => (BControlCommand::SendFirmware { data: m[3..].to_vec() }, m.len()),
                0x35 => (
                ...
                cmd => return Err(ParseError::Invalid(format!("Invalid B-Control command {cmd:x}")))
            };
            let result = BControlSysEx {device, model, command};
            Ok((result, used + 3))
        }
        else{
...
pub enum BControlCommand {
    /// 
    SendBclMessage { text: String },

    RequestIdentity,
    SelectPreset { index: u8 },
    SendFirmware { data: Vec<u8> },
    RequestData { preset: PresetIndex },
    RequestGlobalSetup,
    RequestPresetName { preset: PresetIndex },
    RequestSnapshot,

    SendIdentity { id_string: String },
    BclReply { msg_indx: u16, error_code: u8 },
    SendPresetName { preset: PresetIndex, name: String },
    FirmwareReply { mem_addr: u16, err: u8 },
    SendText
...
impl BControlCommand {
    pub fn extend_midi(&self, v: &mut Vec<u8>) {
        match self {
            BControlCommand::RequestIdentity => { v.push(0x01); },
            ...
            BControlCommand::RequestGlobalSetup => { v.push(0x41); },
            BControlCommand::RequestPresetName { preset } => {
                v.push(0x42); preset.extend_midi(v);
            },
            BControlCommand::RequestSnapshot => { v.push(0x43); },
            BControlCommand::SendIdentity { id_string } => {
                v.push(0x02);
                extend_midi_from_string(id_string, v);
            },
            BControlCommand::BclReply { msg_indx, error_code } => {
                v.push(0x21); 
            ...
            BControlCommand::SendText => { v.push(0x78); },
        }
    }
}
